...
- **Lossless reorder-only mode** — opt-in `--clump_only` applies the same clumping win as a standalone run-and-exit mode WITHOUT any trimming, filtering, or adapter detection. Output records are byte-identical to input records (header + sequence + quality); only file-level order changes. Useful for archival recompression pipelines that need the size win but require record-level fidelity. See [Clump-only](https://www.trimgalore.com/modes/clump-only/) for details
