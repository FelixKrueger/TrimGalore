...
- **Lossless reorder-only mode** — opt-in `--clump_only` applies the same clumping win as a standalone run-and-exit mode WITHOUT any trimming, filtering, or adapter detection. Output records are byte-identical to input records (header + sequence + quality); only file-level order changes. Supports both FASTQ and unaligned BAM (uBAM) in/out via `--output-format ubam`; aux tags round-trip losslessly via `--preserve-tags`. Useful for archival recompression pipelines (including 10X-style single-cell uBAM with `CB`/`UB` tags) that need the size win but require record-level fidelity. See [Clump-only](https://www.trimgalore.com/modes/clump-only/) for details
