...
## This program is Copyright (C) 2012-16, Felix Krueger (felix.krueger@babraham.ac.uk)
...
sub validate_paired_end_files{
  ...
  if ($file_1 =~ /\.gz$/){
    open (IN1,"gunzip -c $output_dir$file_1 |") or die "Couldn't read from file $file_1: $!\n";
  }
  else{
  ...
  if ($file_2 =~ /\.gz$/){
    open (IN2,"gunzip -c $output_dir$file_2 |") or die "Couldn't read from file $file_2: $!\n";
  }
  else{
...
sub file_sanity_check{
  ...
  my $file = shift;
  if ($file =~ /gz$/){
      open (SANITY,"gunzip -c $file |") or die "Failed to read from file '$file' to perform sanity check: $!\n";
  }
  else{
...
sub autodetect_adapter_type{
  ...
  if ($ARGV[0] =~ /gz$/){
    open (AUTODETECT,"gunzip -c $ARGV[0] |") or die "Failed to read from file $ARGV[0]\n";
  }
  else{
  ...
  if ($adapters{$highest}->{count} == $adapters{$second}->{count}){
    warn "Unable to auto-detect most prominent adapter from the first specified file (count $highest: $adapters{$highest}->{count}, count $second: $adapters{$second}->{count})\n";
...
VERSION
  ...
  if (defined $maxn){
	  ...
	  die "--max_n needs to be an integer between 1 and 100\nPlease respecify...\n\n";
      }
  }
