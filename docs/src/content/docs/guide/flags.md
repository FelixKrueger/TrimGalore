...
- **`-a2` / `--adapter2`** warns and is ignored in the same four shapes, for the same underlying reason — there is no Read 2 to trim: under `--hardtrim5`, `--hardtrim3`, `--clock` and `--implicon`, which perform no adapter trimming at all, and on any single-end run. The warning names which of the four applied: `WARNING: -a2/--adapter2 was given but is not used in this mode (<reason>). Ignoring.` Note that in those four shapes the value is *not* parsed either, so a malformed sequence that would abort a paired-end run is discarded without being reported; dropping `--paired` from a working command line therefore silences both the error and the adapter.
...
For library kits where `--rrbs` should **not** be used (Tecan Ovation RRBS Methyl-Seq, MseI-digested libraries), see [When NOT to use `--rrbs`](/modes/rrbs/#when-not-to-use---rrbs).

## Adapter specification recap

See [Multiple adapter sequences](/guide/adapters/#multiple-adapter-sequences) for the three equivalent syntaxes (repeatable `-a`/`-a2`, embedded-string form, and `file:adapters.fa`). Supplementary notes:
...
Plus `A{N}` brace expansion in `-a` / `-a2`. See the [migration guide](/reference/migration/) for the full picture.

## Where to find what

| You want to… | Look here |
|---|---|
| Tune Phred quality trimming | [Quality trimming](/guide/quality/) |
| Specify or override adapters | [Adapter trimming](/guide/adapters/) |
| Rescue under-length pairs | [Paired-end data](/guide/paired-end/) |
| Set length and max-N cutoffs | [Length filtering](/guide/length/) |
| Read a trimming report | [Trimming reports](/guide/reports/) |
| Trim RRBS / bisulfite libraries | [RRBS mode](/modes/rrbs/) and [Bisulfite & RRBS](/rrbs/guide/) |
| Use a specialty UMI mode | [Mouse Epigenetic Clock](/modes/clock/) and [IMPLICON](/modes/implicon/) |
